...
definePageMeta({ title: 'Sunting Draf Permohonan' })
...
const stepLabels = ['Maklumat Syarikat', 'Maklumat Permohonan', 'Semak & Simpan']
...
<template>
  <div class="max-w-2xl mx-auto space-y-4">
    <div class="flex items-center gap-3">
      <UButton :to="`/perlesenan/permohonan-lesen/${$route.params.id}`" icon="i-lucide-arrow-left" color="neutral" variant="ghost" />
      <div>
        <h2 class="text-xl font-bold text-gray-900">Sunting Draf</h2>
        <p class="text-sm text-gray-500">{{ permohonan?.noRujukan }} — Langkah {{ step }} daripada {{ totalSteps }} — {{ stepLabels[step - 1] }}</p>
      </div>
    </div>
    ...
    <UCard>
      ...
      <!-- Step 3: Semak & Simpan -->
      <template v-else>
        <h3 class="font-semibold mb-4">Langkah 3: Semak & Simpan</h3>
        ...
        <dl class="space-y-2 text-sm mb-5">
          ...
          <div class="flex gap-4">
            ...
            <dd class="font-medium">{{ form.alamat }}<template v-if="form.poskod">, {{ form.poskod }}</template> {{ form.negeri }}</dd>
          </div>
        ...
        <UAlert class="mt-4" color="info" variant="subtle" icon="i-lucide-info" description="Anda boleh menyimpan draf atau terus kemukakan permohonan untuk semakan." />
        <div class="flex justify-between mt-6">
          <UButton color="neutral" variant="ghost" icon="i-lucide-arrow-left" @click="step = 2">Kembali</UButton>
          <div class="flex gap-2">
            <UButton color="neutral" variant="subtle" :loading="loading" @click="saveForm(false)">
              <UIcon name="i-lucide-save" class="mr-1" /> Simpan Draf
            </UButton>
            <UButton color="neutral" :loading="loading" @click="saveForm(true)">
